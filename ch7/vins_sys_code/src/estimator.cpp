...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double header)
{
    ...
    // 判断两幅图像之间运动多少像素单位， 如果超过阈值比如10个像素单位，就判断为一个关键帧
    if (f_manager.addFeatureCheckParallax(frame_count, image, td))
        marginalization_flag = MARGIN_OLD;                 /* 丢掉老的关键帧 */
    else
        marginalization_flag = MARGIN_SECOND_NEW;           /* margin 前面图像 */
    ...
    if (ESTIMATE_EXTRINSIC == 2)
    {
        ...
        if (frame_count != 0)
        {
            ...
            if (initial_ex_rotation.CalibrationExRotation(corres, pre_integrations[frame_count]->delta_q, calib_ric))
            {
                ...
                ric[0] = calib_ric;
                RIC[0] = calib_ric;
                ESTIMATE_EXTRINSIC = 1;     /* 标定完成之后，我们认为有了初始值，故将该变量设置为1 */
            }
        }
    }
    ...
    if (solver_flag == INITIAL)
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            if (ESTIMATE_EXTRINSIC != 2 && (header - initial_timestamp) > 0.1)
            {
                ...
                result = initialStructure();               // 初始化的操作, 完成之后将result变成true
                initial_timestamp = header;
            }
            if (result)
            {
                solver_flag = NON_LINEAR;
                solveOdometry();    // 不断推测相机在三维空间中的姿态
                slideWindow();
...
void Estimator::solveOdometry()
{
    ...
    {
        TicToc t_tri;
        f_manager.triangulate(Ps, tic, ric);    // 特征点的三角化
        //cout << "triangulation costs : " << t_tri.toc() << endl;        
        backendOptimization();  // 整个后端的优化
    }
}
...
{
    backend::LossFunction *lossfunction;
    lossfunction = new backend::CauchyLoss(1.0);    // hessian矩阵的权重
